...
"""
Build the 282-feature datasets with optional magnitude and focal-depth filters
for feature construction while retaining labels from the full catalog.

This is a minimal catalog-filter sensitivity variant of the original
01_build_raw282_dataset.py.  For example, with
--feature-depth-min-km 0 --feature-depth-max-km 70, all Annual, Monthly, and
Latest-100 predictors are recomputed using only events with known shallow
focal depths 0 <= d < 70 km.  Future ML >= 5 labels are always generated from
the unfiltered catalog used by the original workflow.  Temporal splits, sample
ordering, feature metadata, and target definitions remain unchanged.
"""
...
def make_feature_catalog(
    ...
    """Return the filtered catalog used only for feature construction.

    The upper depth boundary is exclusive.  The label catalog is deliberately
    kept separate and is never passed through this function.
    """
    mask = np.ones(len(catalog), dtype=bool)

    if min_magnitude is not None:
        mask &= catalog["magnitude"].astype(float).values >= float(min_magnitude)
    if depth_min_km is not None:
        mask &= catalog["depth"].astype(float).values >= float(depth_min_km)
    if depth_max_km is not None:
        mask &= catalog["depth"].astype(float).values < float(depth_max_km)

    out = catalog.loc[mask].copy()
    out = out.sort_values("date_time").reset_index(drop=True)
    return out


def feature_catalog_diagnostics(event_obj, t0, feature_window_days=365):
    """Small audit fields for a filtered feature catalog."""
    ...
    annual_count = int(max(0, right - left))
    prior_count = int(right)
    last100_count = int(min(100, prior_count))

    span_days = 0.0
    if last100_count == 100:
        last_times = times[right - 100:right]
        span_days = float(
    ...
    return {
        "feature_annual_event_count": annual_count,
        "feature_prior_event_count": prior_count,
        "feature_last100_count": last100_count,
        "feature_last100_complete": bool(last100_count == 100),
        "feature_last100_span_days": span_days,
    }
...
def build_one_horizon(
    ...
    for _, prow in tqdm(patch.iterrows(), total=len(patch)):
        ...
        region = prow["region"]
        feature_ev = feature_event_dict[patch_order]
        label_ev = label_event_dict[patch_order]
        ...
        for t0 in feature_dates:
            ...
            feature_diag = feature_catalog_diagnostics(
                feature_ev,
                t0=t0,
                feature_window_days=args.feature_window_days,
            )

            max_mag = future_max_mag(label_ev, t0, H)
            ...
            row = {
                ...
                "y_class": y_class,
                "y_m5": y_m5,
                "feature_min_magnitude": args.feature_min_magnitude,
                "feature_depth_min_km": args.feature_depth_min_km,
                "feature_depth_max_km_exclusive": args.feature_depth_max_km,
            }
            row.update(feature_diag)
...
def main():
    ...
    parser.add_argument(
        "--feature-min-magnitude",
        type=float,
        default=None,
        help=(
            "Minimum magnitude used only to construct catalog features. "
            "Future ML>=5 labels remain based on the full catalog."
        ),
    )
    parser.add_argument(
        "--feature-depth-min-km",
        type=float,
        default=None,
        help=(
            "Inclusive minimum focal depth used only for feature construction. "
            "Future ML>=5 labels remain based on the unfiltered catalog."
        ),
    )
    parser.add_argument(
        "--feature-depth-max-km",
        type=float,
        default=None,
        help=(
            "Exclusive maximum focal depth used only for feature construction. "
            "Future ML>=5 labels remain based on the unfiltered catalog."
        ),
    )
    ...
    if (
        args.feature_depth_min_km is not None
        and args.feature_depth_max_km is not None
        and args.feature_depth_min_km >= args.feature_depth_max_km
    ):
        raise ValueError(
            "--feature-depth-min-km must be smaller than "
            "--feature-depth-max-km"
        )
    ...
    label_catalog = load_catalog(args.catalog_path, patch)
    feature_catalog = make_feature_catalog(
        label_catalog,
        min_magnitude=args.feature_min_magnitude,
        depth_min_km=args.feature_depth_min_km,
        depth_max_km=args.feature_depth_max_km,
    )

    catalog_min = label_catalog["date_time"].min()
    catalog_max = label_catalog["date_time"].max()

    print("patch number:", len(patch), flush=True)
    print("full catalog rows for labels:", len(label_catalog), flush=True)
    print("feature catalog rows:", len(feature_catalog), flush=True)
    print("feature minimum magnitude:", args.feature_min_magnitude, flush=True)
    print("feature depth minimum (inclusive):", args.feature_depth_min_km, flush=True)
    print(
        "feature depth maximum (exclusive):",
        args.feature_depth_max_km,
        flush=True,
    )
    print("catalog time:", catalog_min, "to", catalog_max, flush=True)

    label_event_dict = build_patch_event_arrays(label_catalog, patch)
    feature_event_dict = build_patch_event_arrays(feature_catalog, patch)
    ...
    for H in horizons:
        X, y_class, y_m5, meta = build_one_horizon(
            H=H,
            patch=patch,
            feature_event_dict=feature_event_dict,
            label_event_dict=label_event_dict,
        ...
        payload = {
            ...
            "feature_dim": 282,
            "feature_definition": (
                "[t0 - 365 days, t0), raw unnormalized features; "
                "feature-catalog filters recorded in args"
            ),
            "feature_min_magnitude": args.feature_min_magnitude,
            "feature_depth_min_km": args.feature_depth_min_km,
            "feature_depth_max_km_exclusive": args.feature_depth_max_km,
            "label_catalog_definition": "full catalog; future target is ML >= 5",
        ...
        for subset_name, subset_mask in {
            "train_pool": meta["split"].eq("train_pool").values,
            "test": meta["split"].eq("test").values,
        }.items():
            g = meta.loc[subset_mask]
            diagnostic_rows.append({
                ...
                "feature_depth_min_km": args.feature_depth_min_km,
                "feature_depth_max_km_exclusive": args.feature_depth_max_km,
                "annual_event_count_median": float(g["feature_annual_event_count"].median()) if len(g) else np.nan,
                "last100_complete_fraction": float(g["feature_last100_complete"].mean()) if len(g) else np.nan,
                "last100_span_days_median_complete": float(
                    g.loc[g["feature_last100_complete"], "feature_last100_span_days"].median()
                ) if bool(g["feature_last100_complete"].any()) else np.nan,
            })
        diagnostic_path = os.path.join(out_dir, f"feature_threshold_diagnostics_{H}m.csv")
    ...
    manifest = {
        ...
        "label_catalog_definition": "unfiltered full catalog; future target is ML >= 5",
